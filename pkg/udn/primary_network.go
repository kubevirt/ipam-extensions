...
func FindPrimaryNetwork(ctx context.Context,
	...
	if err := cli.List(ctx, &nadList, client.InNamespace(namespace)); err != nil {
		return nil, fmt.Errorf("failed listing nads for pod namespace")
	}

	var primaryNetworkNAD *v1.NetworkAttachmentDefinition
	for _, nad := range nadList.Items {
		...
		if netConfig.Role == config.NetworkRolePrimary {
			if primaryNetworkNAD != nil {
				return nil, fmt.Errorf("unexpected primary network found, only one primary network should be configured")
			}
			primaryNetworkNAD = ptr.To(nad)
		}
	}
	return primaryNetworkNAD, nil
}
